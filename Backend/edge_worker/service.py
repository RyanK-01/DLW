...
from edge_worker.sync_worker import AlertSyncWorker
...
class MultiCameraEdgeService:
    def __init__(self, cameras: list[CameraConfig], cfg: EdgeInferenceConfig) -> None:
        ...
        self.sync_worker = AlertSyncWorker(
            ...
            alert_endpoint=cfg.alert_endpoint,
            sync_interval_seconds=cfg.sync_interval_seconds,
            db_path=cfg.offline_db_path,
        )
    ...
    def _process_frame(self, cam: CameraConfig, rt: CameraRuntime, frame: np.ndarray, now: datetime) -> None:
        ...
        payload = self._attach_snapshot_and_compat_fields(
            payload=payload,
            frame=evidence_clip_frames[-1] if evidence_clip_frames else None,
            confidence=float(rt.ema_fight_score),
            incident_type="fight",
            camera_id=cam.camera_id,
            location={"latitude": cam.latitude, "longitude": cam.longitude},
            timestamp=now,
    ...
    def _attach_snapshot_and_compat_fields(
        self,
        ...
        if frame is not None:
            snapshot_payload = build_alert_payload(
                frame_bgr=frame,
            ...
            if snapshot_payload:
                enriched["type"] = snapshot_payload["type"]
                enriched["snapshot"] = snapshot_payload["snapshot"]
                enriched["delivery_status"] = snapshot_payload["status"]
                enriched["latestFrameJpeg"] = f"data:image/jpeg;base64,{snapshot_payload['snapshot']}"

        enriched["status"] = "NEW"
        enriched["riskScore"] = float(confidence)
        enriched["category"] = incident_type
        enriched["lat"] = float(location["latitude"])
        enriched["lng"] = float(location["longitude"])
    ...
    def _post_alert(self, payload: dict[str, Any]) -> None:
        self.sync_worker.send_or_buffer(payload)
...
def load_config_from_env() -> EdgeInferenceConfig:
    ...
    def resolve_backend_path(raw_path: Optional[str]) -> Optional[str]:
        if not raw_path:
            return None
        ...
        direct = (backend_dir / candidate).resolve()
        if direct.exists() or not value.lower().startswith("backend/"):
            return str(direct)

        without_backend_prefix = Path(*candidate.parts[1:]) if len(candidate.parts) > 1 else candidate
        fallback = (backend_dir / without_backend_prefix).resolve()
        return str(fallback)
    ...
    return EdgeInferenceConfig(
        ...
        artifacts_root=resolve_backend_path(os.getenv("ARTIFACTS_ROOT", "./artifacts")) or str((backend_dir / "artifacts").resolve()),
        heartbeat_interval_seconds=float(os.getenv("HEARTBEAT_INTERVAL_SECONDS", "5")),
        confidence_threshold=float(os.getenv("CONFIDENCE_THRESHOLD", "0.70")),
        offline_db_path=resolve_backend_path(os.getenv("OFFLINE_DB_PATH", "./buffer.db")) or str((backend_dir / "buffer.db").resolve()),
        sync_interval_seconds=float(os.getenv("SYNC_INTERVAL_SECONDS", "5")),
        alert_endpoint=os.getenv("ALERT_ENDPOINT", "/api/incidents/alert"),
        weapon_weights=resolve_backend_path(os.getenv("WEAPON_WEIGHTS", "best.pt") or None),
        ...
        mobilenet_model_path=resolve_backend_path(os.getenv("MOBILENET_MODEL_PATH") or None),
